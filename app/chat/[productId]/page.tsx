        ...
        <CardContent>
          <ScrollArea className="h-[400px] w-full pr-4">
            {messages.map((message) => (
              <div
                key={message.id}
                className={`flex ${message.sender === "buyer" ? "justify-end" : "justify-start"} mb-4`}
              >
                <div className={`flex items-start ${message.sender === "buyer" ? "flex-row-reverse" : "flex-row"}`}>
                  <Avatar className="w-8 h-8">
                    <AvatarFallback>{message.sender === "buyer" ? "B" : "S"}</AvatarFallback>
                  </Avatar>
                  <div className={`mx-2 ${message.sender === "buyer" ? "text-right" : "text-left"}`}>
                    <div
                      className={`px-4 py-2 rounded-lg ${message.sender === "buyer" ? "bg-green-500 text-white" : "bg-gray-200 dark:bg-gray-700"}`}
                    >
                      {message.content}
                    </div>
